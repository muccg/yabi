...
public class ListCompletedJobs extends BaseAction {

    private static Logger logger;

    public ListCompletedJobs () {
        super();
    }
    ...
    public ActionForward execute(
        ...
        logger = Logger.getLogger( AppDetails.getAppString( request.getContextPath() ) + "." + ListCompletedJobs.class.getName() );
        ...
        try {
            ...
            if (request.getMethod().compareTo("GET") == 0 &&
                ...
                String ageStr = request.getParameter("age").replaceAll("\\.\\.","");

                long age = Long.parseLong(ageStr, 10) * 1000; //convert seconds to milliseconds

                String filePath = rootDirLoc + user + "/.completed";
                ...
                if (requestedFile.exists() && requestedFile.isDirectory()) {
                    ...
                    for (int i=0; i<files.length; i++) {
                        ...
                        if (parts.length < 3) continue; //skip files that don't match our new format as we can't keep track of them properly
                        if (files[i].lastModified() + age < System.currentTimeMillis()) continue; //skip files older than the requested age
                    ...
                    request.setAttribute("message", "error accessing completed dirs");
                ...
                request.setAttribute("message", "list must be performed via a GET operation, and required params (user, age) must be specified");
