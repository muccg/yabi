...
class FileDeleteResource(resource.PostableResource):
    ...
    def http_POST(self, request):
        ...
        def DeleteCommand(res):
            ...
            delete=bend.http_DELETE(request,path=[username]+pathremainder, username=username,recurse=recurse)
            ...
            if isinstance(delete,defer.Deferred):
                ...
                delete.addCallback(_rm_done)
                delete.addErrback( lambda r: client_channel.callback(http.Response( responsecode.INTERNAL_SERVER_ERROR, {'content-type': http_headers.MimeType('text', 'plain')}, "NOT OK: %s\n"%str(r)) ))
            else:
                client_channel.callback(delete)
                
        # on success, make the dir
        deferred.addCallback(DeleteCommand)
