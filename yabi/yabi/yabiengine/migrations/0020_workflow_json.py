...
def munge_json(munge, text, ob, Tool):
    if text:
        name = "%s id=%s" % (type(ob).__name__, ob.id)
        ...
        if d:
            text = json.dumps(munge(d, Tool, name))
        else:
            logger.warning("Couldn't parse json on %s" % name)

    return text


def add_toolid(ob, Tool, name):
    "Adds toolId entries to each job in workflow json"
    for job in ob.get("jobs", []):
        if "toolId" not in job:
            toolName = job.get("toolName", "")
            tools = Tool.objects.filter(desc__name=toolName)
            if len(tools) > 0:
                job["toolId"] = tools[0].id
            else:
                logger.warning("Couldn't find a tool with name \"%s\" for %s" % (toolName, name))
    return ob


def remove_toolid(ob, Tool, name):
    "Get rid of toolId entries from each job in workflow json"
    for job in ob.get("jobs", []):
        if "toolId" in job:
            del job["toolId"]
    return ob
...
class Migration(DataMigration):
    depends_on = (
        ("yabi", "0027_auto__chg_field_tool_path"),
    )

    def forwards(self, orm):
        "Write your forwards methods here."
        for workflow in orm['yabiengine.Workflow'].objects.all():
            workflow.original_json = munge_json(add_toolid, workflow.original_json,
                                                workflow, orm['yabi.Tool'])
            workflow.save()

        for workflow in orm['yabiengine.SavedWorkflow'].objects.all():
            workflow.json = munge_json(add_toolid, workflow.json,
                                       workflow, orm['yabi.Tool'])
            workflow.save()
    ...
    def backwards(self, orm):
        ...
        for workflow in orm['yabiengine.Workflow'].objects.all():
            workflow.original_json = munge_json(remove_toolid, workflow.original_json,
                                                workflow, orm['yabi.Tool'])
            workflow.save()

        for workflow in orm['yabiengine.SavedWorkflow'].objects.all():
            workflow.json = munge_json(remove_toolid, workflow.json,
                                       workflow, orm['yabi.Tool'])
            workflow.save()
