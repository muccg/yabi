...
class SSHTorqueBackendTest(YabiTestCase):
    ...
    def setUp(self):
        ...
        admin.create_sshtorque_backend()
        admin.create_tool('hostname', ex_backend_name='SSHTorque Backend')
        admin.add_tool_to_all_tools('hostname')

    def tearDown(self):
        models.Tool.objects.get(desc__name='hostname').delete()
        models.Backend.objects.get(name='SSHTorque Backend').delete()
    ...
    def test_a_failure_to_get_host_key(self):
        ...
        result = self.yabi.run(['hostname', '--backend', 'SSHTorque Backend'])
    ...
    def test_submit_json_directly_larger_workflow(self):
        result = self.yabi.run(['submitworkflow', '--backend', 'SSHTorque Backend',
