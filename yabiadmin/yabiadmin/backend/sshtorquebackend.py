...
from yabiadmin.yabiengine.urihelper import uriparse
from yabiadmin.backend.torqueparsers import TorqueParser, TorqueQStatResult, TorqueQSubResult
...
class SSHTorqueExecBackend(SSHBackend):
    QSUB_TEMPLATE = "\n".join(["#!/bin/sh",
    ...
    def submit_task(self):
        logger.debug("SSHTorqueExecBackend.submit_task ...")
        qsub_result = self._run_qsub()

        if qsub_result.status == qsub_result.JOB_SUBMITTED:
            self.task.remote_id = qsub_result.remote_id
            self.task.save()
            logger.debug("Yabi Task %s submitted to Torque OK. remote id = %s" % (self.task.pk, self.task.remote_id))
        else:
            raise Exception("Error submitting remote job to Torque for yabi task %s %s" % (self.task.pk, qsub_result.status))
    ...
    def _run_qsub(self):
        ...
        qsub_script_body = SSHTorqueExecBackend.QSUB_TEMPLATE.format(submission_script_name, submission_script_body)
    ...
    def poll_task_status(self):
        ...
        if qstat_result.status == TorqueQStatResult.JOB_RUNNING:
        ...
        elif qstat_result.status == TorqueQStatResult.JOB_NOT_FOUND:
        ...
        elif qstat_result.status == TorqueQStatResult.JOB_SUCCEEDED:
            logger.debug("yabi task %s succeeded" % self.task.pk)

        elif qstat_result.status == TorqueQStatResult.JOB_FAILED:
