...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Copy Tool fields into ToolDesc."
        ToolDesc = orm['yabi.ToolDesc']
        Tool = orm['yabi.Tool']
        ...
        for tool in Tool.objects.all():
            ...
            desc.save()
            mapping[tool.id] = desc.id

            tool.desc_id = desc.id
            tool.save()

        for model in ('ToolParameter', 'ToolOutputExtension', 'ToolGrouping'):
            for ob in orm['yabi.' + model].objects.all():
                ob.tool_desc_id = mapping[ob.tool_id]
                ob.save()

    def backwards(self, orm):
        "Copy ToolDesc fields into Tool."
        Tool = orm['yabi.Tool']
        ...
        for model in ('ToolParameter', 'ToolOutputExtension', 'ToolGrouping'):
            for ob in orm['yabi.' + model].objects.all():
                ob.tool_id = Tool.objects.filter(desc_id=ob.tool_desc_id)[0].id
                ob.save()
